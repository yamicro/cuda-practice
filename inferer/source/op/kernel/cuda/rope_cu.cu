...
namespace kernel {
    ...
    __global__ void sin_cos_calc_cu(int head_size, int max_seq_len, float* sin_cache, float* cos_cache) {
        int idx = threadIdx.x + blockDim.x * blockIdx.x;
    ...
    __global__ void rope_kernel_cu_fp32(int pos, int dim, int head_size,
                                        const float* input_q, const float* input_k) {
        int idx = threadIdx.x + blockDim.x * blockIdx.x;
        idx = idx * 2;
        if (idx >= dim) {
    ...
    void sin_cos_cache_calc_cu(int head_size, int max_seq_len, const tensor::Tensor& sin_cache,
        ...
        if (stream) {
            sin_cos_calc_cu<<<1, threads, 0, stream>>>(head_size, max_seq_len,
                                                    const_cast<float*>(sin_cache.ptr<float>()),
                                                    const_cast<float*>(cos_cache.ptr<float>()));
        } else {
            sin_cos_calc_cu<<<1, threads>>>(head_size, max_seq_len, const_cast<float*>(sin_cache.ptr<float>()),
                                         const_cast<float*>(cos_cache.ptr<float>()));
        }
    }
}